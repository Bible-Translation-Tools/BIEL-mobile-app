...
export function BookHeaderIcon({ size }: BookHeaderIconProps) {
  ...
  return (
    <Svg width={size} height={height} viewBox={`0 0 ${VIEWBOX_WIDTH} ${VIEWBOX_HEIGHT}`} fill="none">
      <Path
        d={BOOK_PATH}
        fill={isDark ? 'white' : 'url(#bookHeaderGradient)'}
      />
      {!isDark && (
        <Defs>
          <LinearGradient
            id="bookHeaderGradient"
            x1="75.3351"
            y1="0.188324"
            x2="75.3351"
            y2="128.301"
            gradientUnits="userSpaceOnUse">
            <Stop stopColor="#3C9FFE" />
            <Stop offset={1} stopColor="#0274DF" />
          </LinearGradient>
        </Defs>
      )}
    </Svg>
